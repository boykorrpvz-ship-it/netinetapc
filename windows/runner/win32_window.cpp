...
bool Win32Window::Create(const std::wstring& title,
  ...
  // Fixed-size window: keep the title bar, minimize and close, but drop the
  // resize border and maximize box so the window stays a static size.
  const DWORD window_style =
      WS_OVERLAPPEDWINDOW & ~(WS_THICKFRAME | WS_MAXIMIZEBOX);

  // Treat |size| as the desired client (content) area and expand it to the
  ...
  // Center the window on the monitor work area.
  int window_x = Scale(origin.x, scale_factor);
  int window_y = Scale(origin.y, scale_factor);
  MONITORINFO monitor_info = {sizeof(MONITORINFO)};
  if (GetMonitorInfo(monitor, &monitor_info)) {
    window_x = static_cast<int>(
        monitor_info.rcWork.left +
        ((monitor_info.rcWork.right - monitor_info.rcWork.left) - window_width) /
            2);
    window_y = static_cast<int>(
  ...
  HWND window = CreateWindow(
      window_class, title.c_str(), window_style, window_x, window_y,
